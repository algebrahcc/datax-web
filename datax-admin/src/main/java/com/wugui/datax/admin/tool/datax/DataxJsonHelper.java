...
public class DataxJsonHelper implements DataxJsonInterface {
    ...
    private TxtfileReaderDto txtfileReaderDto;
    ...
    public void initReader(DataXJsonBuildDto dataxJsonDto, JobDatasource readerDatasource) {
        ...
        this.txtfileReaderDto = dataxJsonDto.getTxtfileReader();
        // reader 插件
    ...
    @Override
    public Map<String, Object> buildTxtfileReader() {
        DataxTxtFilePojo dataxTxtFilePojo = new DataxTxtFilePojo();
        dataxTxtFilePojo.setJdbcDatasource(readerDatasource);
        List<Map<String, Object>> columns = Lists.newArrayList();

        readerColumns.forEach(c -> {
            c = c.replaceAll("\"", "");
            Map<String, Object> column = Maps.newLinkedHashMap();
            String index = c.split(Constants.SPLIT_SCOLON)[1].split(",")[0].replace("\\","");
            String type = c.split(Constants.SPLIT_SCOLON)[2].split("}")[0].split(",")[0].replace("\\","");
            column.put("index", index);
            column.put("type", type);
            if (Objects.equals(type, "date")) {
                column.put("format","yyyy-MM-dd");
            }
            columns.add(column);
        });
        dataxTxtFilePojo.setColumns(columns);
        dataxTxtFilePojo.setReaderFieldDelimiter(txtfileReaderDto.getReaderFieldDelimiter());
        dataxTxtFilePojo.setReaderPath(txtfileReaderDto.getReaderPath());
        dataxTxtFilePojo.setSkipHeader(txtfileReaderDto.getReaderSkipHeader());
        return readerPlugin.buildTxtFile(dataxTxtFilePojo);
    }
