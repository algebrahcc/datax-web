...
public class QueryToolFactory {
    ...
    public static BaseQueryTool getByDbType(JobDatasource jobDatasource) {
        ...
        }else if (JdbcConstants.TXTFILE.equals(datasource)){
            return getTxtFileQueryToolInstance(jobDatasource);
        }
    ...
    private static BaseQueryTool getTxtFileQueryToolInstance(JobDatasource jdbcDatasource) {
        try {
            return new TxtFileQueryTool(jdbcDatasource);
        } catch (SQLException e) {
            throw RdbmsException.asConnException(JdbcConstants.TXTFILE,
                    e, jdbcDatasource.getJdbcUsername(), jdbcDatasource.getDatasourceName());
        }
    }
}
