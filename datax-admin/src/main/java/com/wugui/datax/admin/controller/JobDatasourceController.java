...
public class JobDatasourceController extends BaseController {
    ...
    public R<Boolean> delete(@RequestParam("id") Long id) {
        ...
        String fileAddress = this.jobJdbcDatasourceService.query().select("file_address").eq("id", id).one().getFileAddress();
        if (fileAddress != null && !fileAddress.isEmpty()) {
            File file = new File(fileAddress);
            if (file.exists()) {
                if (file.isDirectory()) {
        ...
        return success(this.jobJdbcDatasourceService.removeById(id));
    }
